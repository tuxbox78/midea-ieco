...
- `midea-local.json` ist eine kurzlebige 0600-Datei, die nur während einer Cloud-Abfrage existiert, um dem `midealocal`-CLI die Zugangsdaten über eine Config-Datei statt über die Prozess-Kommandozeile zu übergeben (damit das Passwort nicht in `ps` sichtbar ist). `midea_refresh_tokens.py` legt sie in einem privaten, pro Aufruf frischen Temp-Verzeichnis an und entfernt sie sofort; die einmalige Geräteerkennung in `install.sh` legt sie im Installationsverzeichnis an und entfernt sie beim Beenden. Der Name ist als Sicherheitsnetz **git-ignoriert**.
